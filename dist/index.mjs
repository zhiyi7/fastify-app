...
async function discoverRouteFiles(appDirectory) {
	return (await fastGlob("**/*.{js,mjs,ts}", {
		cwd: appDirectory,
		onlyFiles: true,
		absolute: true
	})).filter((apiFile) => !path.basename(apiFile).startsWith("_")).sort((left, right) => left.localeCompare(right));
}
...
async function init(config) {
	...
	fastifyInstance = app;
	if (!appConfig.disableCors) app.register(cors, {
		...defaultCorsOptions,
	...
	const importedModules = await Promise.all(routeEntries.map(({ apiFile }) => import(pathToFileURL(apiFile).href)));
...
//#endregion
export { ApiError, instanceProxy as default, init, start };
